...
def evaluate(model, loader, mode, cfg, multiplier=None):
    ...
    with torch.no_grad():
        for img, dimg, mask, id in loader:
            
            img = img.cuda()
                
            
            if mode == 'sliding_window':
                grid_w, grid_h = cfg['crop_size'][0], cfg['crop_size'][1]
                b, _, h, w = img.shape
                final = torch.zeros(b, 40, h, w).cuda()

                row = 0
                while row < h:
                    col = 0
                    while col < w:
                        t_img = img[:, :, row: row + grid_h, col: col + grid_w]
                        t_dimg = dimg[:, :, row: row + grid_h, col: col + grid_w]
                        pred = model([t_img, t_dimg])
                        final[:, :, row: row + grid_h, col: col + grid_w] += pred.softmax(dim=1)
                        if col == w - grid_w:
                            break
                        col = min(col + int(grid_w * 2 / 3), w - grid_w)
                    if row == h - grid_h:
                        break
                    row = min(row + int(grid_h * 2 / 3), h - grid_h)
                ...
                if multiplier is not None:
                    ...
                    dimg = F.interpolate(dimg, (new_h, new_w), mode='bilinear', align_corners=True)
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, (img, depth_img, mask) in enumerate(trainloader):
